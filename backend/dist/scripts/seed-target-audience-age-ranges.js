...
async function seedTargetAudienceAgeRanges() {
    try {
        ...
        await TargetAudienceAgeRange_1.default.deleteMany({});
        console.log('Cleared existing target audience age ranges');
        // Insert age ranges
        const result = await TargetAudienceAgeRange_1.default.insertMany(ageRanges, { ordered: false });
        console.log(`Successfully seeded ${result.length} target audience age ranges`);
        // Display all seeded age ranges
        const allAgeRanges = await TargetAudienceAgeRange_1.default.find({}).sort('min');
