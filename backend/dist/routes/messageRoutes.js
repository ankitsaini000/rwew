...
router.post('/test-send', async (req, res) => {
    try {
        ...
        let sender = null;
        if (senderEmail) {
            sender = await User_1.default.findOne({ email: senderEmail });
        }
        if (!sender) {
            // Create a temporary sender if none provided
            sender = await User_1.default.create({
...
// @access  Private
router.post('/', auth_1.protect, cloudinary_1.upload.single('file'), messageController_1.sendMessage);
...
// Conversation routes
router.post('/conversations', auth_1.protect, messageController_1.createConversation);
router.post('/conversations/:conversationId/archive', auth_1.protect, messageController_1.archiveConversation);
router.delete('/conversations/:conversationId', auth_1.protect, messageController_1.deleteConversation);
// Message routes
router.get('/conversations/:conversationId/messages', auth_1.protect, messageController_1.getMessages);
// Add the recent messages route
router.get('/recent', messageController_1.getRecentMessages);
