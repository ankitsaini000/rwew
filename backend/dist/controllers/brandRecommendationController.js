...
exports.getSmartRecommendations = exports.refreshRecommendations = exports.getAutomaticRecommendations = void 0;
...
const generateAutomaticRecommendations = async (brandId) => {
    try {
        ...
        const brandProfile = await BrandProfile_1.default.findOne({ userId: brandId });
        ...
        if (brandProfile === null || brandProfile === void 0 ? void 0 : brandProfile.industry) {
        ...
        const [topRatedCreators, trendingCreators, diverseCreators, newCreators] = await Promise.all([
            // Top rated creators
            CreatorProfile_1.CreatorProfile.find(recommendationCriteria)
            ...
            CreatorProfile_1.CreatorProfile.find(Object.assign(Object.assign({}, recommendationCriteria), { 'professionalInfo.categories': { $in: ['Technology', 'Beauty', 'Fitness', 'Travel', 'Food'] } }))
        ...
        const uniqueCreators = allCreators.filter((creator, index, self) => index === self.findIndex(c => c._id.toString() === creator._id.toString()));
 ...
 */
const applyDiversityAlgorithm = (creators, limit) => {
    var _a, _b, _c, _d;
 ...
 */
exports.getAutomaticRecommendations = (0, express_async_handler_1.default)(async (req, res) => {
    ...
    try {
        ...
        const populatedRecommendation = await BrandRecommendation_1.default.findById(brandRecommendation._id)
            .populate({
        ...
        res.status(200).json({
            success: true,
            data: (populatedRecommendation === null || populatedRecommendation === void 0 ? void 0 : populatedRecommendation.recommended_creators) || []
        });
    }
    catch (error) {
 ...
 * @access  Private (Brand only)
 */
exports.refreshRecommendations = (0, express_async_handler_1.default)(async (req, res) => {
    const brandId = req.user._id.toString(); // Convert ObjectId to string
    try {
        // Generate new recommendations
        const recommendedCreatorIds = await generateAutomaticRecommendations(brandId);
        // Update or create recommendations
        const brandRecommendation = await BrandRecommendation_1.default.findOneAndUpdate({ brand_id: brandId }, { recommended_creators: recommendedCreatorIds }, { upsert: true, new: true }).populate({
            path: 'recommended_creators',
            select: '-__v',
            populate: {
                path: 'userId',
 ...
 * @access  Private (Brand only)
 */
exports.getSmartRecommendations = (0, express_async_handler_1.default)(async (req, res) => {
    const brandId = req.user._id.toString(); // Convert ObjectId to string
    try {
        ...
        if (preference) {
            ...
            const brandRecommendation = await BrandRecommendation_1.default.findOne({ brand_id: brandId });
            if (!brandRecommendation) {
                // Generate new automatic recommendations
                const recommendedCreatorIds = await generateAutomaticRecommendations(brandId);
                const newRecommendation = new BrandRecommendation_1.default({
                    brand_id: brandId,
                    recommended_creators: recommendedCreatorIds
                });
                await newRecommendation.save();
                const populatedRecommendation = await BrandRecommendation_1.default.findById(newRecommendation._id)
                    .populate({
                    path: 'recommended_creators',
                    select: '-__v',
                    populate: {
