...
exports.createTestResponseData = exports.getSocialMetrics = exports.getDashboardMetrics = exports.updatePerformanceData = exports.updateCreatorMetrics = exports.getDashboardData = void 0;
...
const PromotionRevenue_1 = __importDefault(require("../models/PromotionRevenue"));
...
exports.getDashboardData = asyncHandler(async (req, res, next) => {
    ...
    if (((_b = req.user) === null || _b === void 0 ? void 0 : _b.role) !== 'creator') {
        res.status(403);
    ...
    const recentOrders = await Order_1.default.find({ creator: creatorId })
        .sort({ date: -1 })
        .limit(5)
        .lean();
    ...
    const revenueByPromotion = await PromotionRevenue_1.default.find({
        creator: creatorId,
        year: currentYear,
        month: currentMonth
    }).lean();
    ...
    const previousMonthRevenue = await PromotionRevenue_1.default.find({
        creator: creatorId,
        year: previousYear,
        month: previousMonth
    }).lean();
    ...
    const dashboardData = {
        creatorInfo: {
            username: ((_c = creatorProfile === null || creatorProfile === void 0 ? void 0 : creatorProfile.personalInfo) === null || _c === void 0 ? void 0 : _c.username) || '',
            name: `${((_d = creatorProfile === null || creatorProfile === void 0 ? void 0 : creatorProfile.personalInfo) === null || _d === void 0 ? void 0 : _d.firstName) || ''} ${((_e = creatorProfile === null || creatorProfile === void 0 ? void 0 : creatorProfile.personalInfo) === null || _e === void 0 ? void 0 : _e.lastName) || ''}`.trim() || 'Creator',
 ...
 * @access  Private (Creator only)
 */
exports.updateCreatorMetrics = asyncHandler(async (req, res, next) => {
 ...
 * @access  Private (Creator only)
 */
exports.updatePerformanceData = asyncHandler(async (req, res, next) => {
    const creatorId = req.user._id;
    // Check if the user is a creator
    if (req.user.role !== 'creator') {
        res.status(403);
    ...
    if (!creatorMetrics) {
        ...
        if (views || likes || messages || earnings) {
            ...
            if (currentDates.length === 0 ||
                ...
                const maxLength = Math.max(views ? views.length : 0, likes ? likes.length : 0, messages ? messages.length : 0, earnings ? earnings.length : 0, currentDates.length);
 ...
 */
function generateDateArray(length) {
 ...
 */
function getServiceTierRequirements(tier) {
    switch (tier) {
        case 'VIP':
            return {
...
exports.getDashboardMetrics = asyncHandler(async (req, res, next) => {
    ...
    console.log('Creator Profile metrics:', (creatorProfile === null || creatorProfile === void 0 ? void 0 : creatorProfile.metrics) || 'No metrics found');
    // Get profile metrics from creatorprofiles collection
    let profileViews = 0;
    let repeatClientRate = 0;
    let profileCompleteness = 0;
    let dashboardImpressions = 0;
    if (creatorProfile === null || creatorProfile === void 0 ? void 0 : creatorProfile.metrics) {
    ...
    if (creatorMetrics === null || creatorMetrics === void 0 ? void 0 : creatorMetrics.dashboardImpressions) {
    ...
    const orders = await Order_1.default.find({ creator: creatorId }).lean();
    ...
    if (repeatClientRate === 0 && orders.length > 0) {
        ...
        if (creatorProfile) {
            await CreatorProfile_1.CreatorProfile.updateOne({ userId: creatorId }, {
                $set: {
    ...
    if (!creatorProfile) {
        ...
        try {
            const newCreatorProfile = new CreatorProfile_1.CreatorProfile({
                userId: creatorId,
                status: 'draft',
                metrics: {
 ...
 * @access  Private (Creator only)
 */
exports.getSocialMetrics = asyncHandler(async (req, res, next) => {
    const creatorId = req.user._id;
    // Check if the user is a creator
    if (req.user.role !== 'creator') {
        res.status(403);
    ...
    let primaryPlatform;
    let totalReach = 0;
    if (creatorProfile === null || creatorProfile === void 0 ? void 0 : creatorProfile.socialMedia) {
        ...
        if (platformEntries.length > 0) {
            primaryPlatform = platformEntries.reduce((max, current) => current[1] > max[1] ? current : max)[0];
 ...
 */
function getFollowerCount(socialMedia, platform) {
    var _a, _b, _c, _d, _e;
    if (!socialMedia)
        return 0;
    switch (platform) {
        case 'instagram':
            return ((_a = socialMedia.instagram) === null || _a === void 0 ? void 0 : _a.followers) || 0;
        case 'youtube':
            return ((_b = socialMedia.youtube) === null || _b === void 0 ? void 0 : _b.subscribers) || 0;
        case 'twitter':
            return ((_c = socialMedia.twitter) === null || _c === void 0 ? void 0 : _c.followers) || 0;
        case 'facebook':
            return ((_d = socialMedia.facebook) === null || _d === void 0 ? void 0 : _d.followers) || 0;
        case 'linkedin':
            return ((_e = socialMedia.linkedin) === null || _e === void 0 ? void 0 : _e.connections) || 0;
        default:
            return 0;
    }
}
 ...
 */
function calculateInfluencerTierProgress(followers) {
    if (followers >= 100000) {
...
function calculateProfileCompleteness(profile) {
    ...
    if (profile.professionalInfo) {
        ...
        const filledFields = professionalFields.filter(field => profile.professionalInfo[field] &&
    ...
    if (profile.socialMedia) {
        const platforms = ['instagram', 'youtube', 'twitter', 'facebook', 'linkedin'];
        const filledPlatforms = platforms.filter(platform => profile.socialMedia[platform] &&
            profile.socialMedia[platform].username).length;
 ...
 */
function calculateInfluencerTier(followers) {
    if (followers >= 100000) {
        return 'VIP';
    }
    else if (followers >= 50000) {
 ...
 */
function calculateServiceTier(completedProjects, responseRate) {
 ...
 */
async function calculateAverageResponseTime(creatorId) {
    try {
 ...
 */
async function calculateResponseRate(creatorId) {
    try {
        // Get all conversations where the creator is a participant
        const conversations = await Conversation_1.default.find({
            participants: creatorId
 ...
 * @access  Private (Creator only)
 */
exports.createTestResponseData = asyncHandler(async (req, res, next) => {
    const creatorId = req.user._id;
    // Check if the user is a creator
    if (req.user.role !== 'creator') {
        res.status(403);
    ...
    try {
        ...
        const testConversation = new Conversation_1.default({
            participants: [creatorId, new mongoose_1.default.Types.ObjectId()], // Creator and a test user
        ...
        const messages = [
            // Message from other user at 10:00 AM
            {
                ...
                sender: new mongoose_1.default.Types.ObjectId(), // Other user
                receiver: creatorId,
            ...
            // Response from creator at 10:30 AM (30 minutes later)
            {
                conversation: testConversation._id,
                sender: creatorId,
                receiver: new mongoose_1.default.Types.ObjectId(),
            ...
            {
                conversation: testConversation._id,
                sender: new mongoose_1.default.Types.ObjectId(),
                receiver: creatorId,
        ...
        await Message_1.default.insertMany(messages);
