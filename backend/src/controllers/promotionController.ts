...
export const createPromotion = asyncHandler(async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (promotion) {
      ...
      (async () => {
        try {
          const matchingCreators = await CreatorProfile.find({
            'professionalInfo.categories': { $in: promotion.category }
          }).select('userId');

          const brandUser = await User.findById(promotion.brandId).select('fullName username avatar');
          ...
          for (const creator of matchingCreators) {
            ...
            io.to(creator.userId.toString()).emit('newNotification', {
              notification: {
                ...
                fromUser: {
                  ...
                  fullName: brandUser?.fullName || brandUser?.username || 'Brand',
                  avatar: brandUser?.avatar || null
                }
              }
            });
            ...
            try {
              const creatorUser = await User.findById(creator.userId).select('email fullName');
