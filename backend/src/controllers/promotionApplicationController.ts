 ...
 */
export const applyToPromotion = asyncHandler(async (req: Request, res: Response): Promise<any> => {
  try {
    ...
    if (application) {
      ...
      (async () => {
        try {
          ...
          const promotion = await Promotion.findById(promotionId);
          if (!promotion) return;
          const brandUser = await User.findById(promotion.brandId).select('email fullName');
          // Get creator info
          const creatorUser = await User.findById(req.user._id).select('fullName');
          // In-app notification
          const Notification = require('../models/Notification').default || require('../models/Notification');
          const notification = await Notification.create({
            user: promotion.brandId,
            type: 'promotion',
            message: `${creatorUser?.fullName || 'A creator'} applied to your promotion: ${promotion.title}`,
          ...
          if (brandUser && brandUser.email) {
            ...
            await sendEmail({
              ...
              message: `Hi ${brandUser.fullName || ''},<br><br>
                ${creatorUser?.fullName || 'A creator'} has applied to your promotion: <b>${promotion.title}</b>.<br><br>
