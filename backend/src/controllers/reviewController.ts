...
export const createReview = asyncHandler(async (req: Request, res: Response) => {
  ...
  const profile = await CreatorProfile.findOne({ userId: order.creator }) as any;
  if (profile) {
    ...
    const existingReviewIndex = profile.reviews.findIndex((r: any) => r.orderId?.toString() === orderId.toString());
  ...
  const creatorMetrics = await CreatorMetrics.findOne({ creator: order.creator }) as any;
  if (creatorMetrics) {
    if (!Array.isArray(creatorMetrics.reviews)) creatorMetrics.reviews = [];
    ...
    const existingMetricsReviewIndex = creatorMetrics.reviews.findIndex((r: any) => r.orderId?.toString() === orderId.toString());
  ...
  try {
    ...
    const brandUser = req.user as any;
...
export const getCreatorReviews = asyncHandler(async (req: Request, res: Response) => {
  try {
    ...
    let reviews = await Review.find({ creatorId })
...
export const updateReview = asyncHandler(async (req: Request, res: Response) => {
  ...
  const profile = await CreatorProfile.findOne({ userId: review.creatorId }) as any;
  if (profile) {
    if (!Array.isArray(profile.reviews)) profile.reviews = [];
    ...
    const existingReviewIndex = profile.reviews.findIndex((r: any) => r.orderId?.toString() === review.orderId.toString());
  ...
  // Also add or update the review in the creator's metrics (creatormetrics collection)
  const creatorMetrics = await CreatorMetrics.findOne({ creator: review.creatorId }) as any;
  if (creatorMetrics) {
    if (!Array.isArray(creatorMetrics.reviews)) creatorMetrics.reviews = [];
    ...
    // Check if review already exists in creator metrics
    const existingMetricsReviewIndex = creatorMetrics.reviews.findIndex((r: any) => r.orderId?.toString() === review.orderId.toString());
...
// @access  Private (brand)
export const getReviewByOrderId = asyncHandler(async (req: Request, res: Response) => {
...
export const addReviewReply = asyncHandler(async (req: Request, res: Response) => {
  const { text } = req.body;
  ...
  try {
    const Notification = mongoose.model('Notification');
    const creatorUser = req.user as any;
...
// @access  Private (creator)
export const updateReviewReply = asyncHandler(async (req: Request, res: Response) => {
  const { text } = req.body;
  const creatorId = req.user._id;
...
// @access  Private (creator)
export const deleteReviewReply = asyncHandler(async (req: Request, res: Response) => {
  const creatorId = req.user._id;
...
export const cleanupEmptyReplies = asyncHandler(async (req: Request, res: Response) => {
  try {
    // Find all reviews with empty or invalid reply objects
    const reviewsWithEmptyReplies = await Review.find({
      $or: [
...
// @access  Private (brand)
export const getBrandReviews = asyncHandler(async (req: Request, res: Response) => {
  try {
    const brandId = req.user._id;
