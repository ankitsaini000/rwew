...
import DeactivatedAccount from '../models/DeactivatedAccount';
 ...
 * @access  Private
 */
export const syncProfileImage = asyncHandler(async (req: Request, res: Response) => {
  try {
...
export const getUserById = asyncHandler(async (req: Request, res: Response) => {
  const user = await User.findById(req.params.id).select('_id fullName username email avatar role profileImage');
  if (user) {
    res.json(user);
  } else {
    res.status(404);
...
// @access  Private
export const deactivateUserAccount = asyncHandler(async (req: Request, res: Response) => {
  const userId = req.user._id;
  ...
  if (user.role === 'creator') {
    await CreatorProfile.findOneAndUpdate(
      { userId: user._id },
      { isActive: false, status: 'suspended', deactivatedAt: user.deactivatedAt, deactivationReason: reason || 'Deactivated by user' }
    );
  }

  // Store in DeactivatedAccount collection
  await DeactivatedAccount.create({
    userId: user._id,
    email: user.email,
    username: user.username,
    fullName: user.fullName,
...
export const getUserDevices = asyncHandler(async (req: Request, res: Response) => {
  const sessions = await DeviceSession.find({ userId: req.user._id });
  res.json(sessions);
});
...
// @access  Private
export const deleteUserDevice = asyncHandler(async (req: Request, res: Response) => {
  const { sessionId } = req.params;
  await DeviceSession.deleteOne({ _id: sessionId, userId: req.user._id });
...
export const getUsersList = asyncHandler(async (req: Request, res: Response) => {
  const users = await User.find({}, '_id name email username role').limit(10);
  res.json(users);
}); 
