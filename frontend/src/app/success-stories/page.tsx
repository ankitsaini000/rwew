...
const mockStories = [
  ...
  },
  {
    id: 2,
    brand: "FitFuel",
    brandLogo: "/uploads/images/logo2.jpg",
    creator: "@fitwithmike",
    creatorAvatar: "/uploads/images/creator2.jpg",
    platform: "YouTube",
    industry: "Fitness",
    brandType: "Startup",
    ...
    images: ["/uploads/images/guide1.jpg"],
    video: "",
  ...
  return (
    <div className="bg-gray-50 min-h-screen flex flex-col">
      ...
      <section className="w-full bg-gradient-to-b from-purple-700 via-purple-300 to-white text-white py-16 px-4 flex flex-col items-center relative">
        <h1 className="text-4xl md:text-5xl font-bold mb-4 text-center">Success Stories</h1>
        <p className="max-w-2xl text-center text-lg md:text-xl mb-8">Real results from real collaborations. See how brands and creators achieve measurable growth on our platform.</p>
          ...
          {filteredStories.map(story => (
            ...
            >
              <div className="flex items-center gap-3 mb-2">
                <img src={story.creatorAvatar} alt="creator" className="w-10 h-10 rounded-full object-cover border-2 border-purple-200" />
                <span className="font-semibold text-purple-700">{story.creator}</span>
                <img src={story.brandLogo} alt="brand" className="w-8 h-8 rounded object-contain ml-auto" />
              </div>
              <div className="font-bold text-purple-700 text-lg mb-1">{story.headline}</div>
              <div className="text-gray-700 mb-2">{story.summary}</div>
              <div className="text-xs text-purple-500 font-medium mb-1">{story.metrics}</div>
              <div className="flex flex-wrap gap-2 text-xs text-gray-500">
                <span>{story.platform}</span>
                <span>{story.industry}</span>
                <span>{story.brandType}</span>
              </div>
            </button>
          ))}
        </div>
        {filteredStories.length === 0 && <div className="text-center text-gray-500 py-12">No stories found for selected filters.</div>}
      </section>
      {/* Story Details Modal */}
      {modalStory && (
        <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg shadow-lg max-w-lg w-full p-8 relative">
            <button className="absolute top-4 right-4 text-gray-400 hover:text-purple-600" onClick={() => setModalStory(null)}><X size={24} /></button>
            <div className="flex items-center gap-3 mb-4">
              <img src={modalStory.creatorAvatar} alt="creator" className="w-12 h-12 rounded-full object-cover border-2 border-purple-200" />
              <span className="font-semibold text-purple-700">{modalStory.creator}</span>
              <img src={modalStory.brandLogo} alt="brand" className="w-10 h-10 rounded object-contain ml-auto" />
            </div>
            <img src={modalStory.images?.[0] || "/uploads/images/guide1.jpg"} alt="story" className="w-full h-40 object-cover rounded mb-4" />
            <div className="font-bold text-purple-700 text-2xl mb-2">{modalStory.headline}</div>
            <div className="text-gray-700 mb-2">{modalStory.summary}</div>
            <div className="text-xs text-purple-500 font-medium mb-1">{modalStory.metrics}</div>
            <div className="mb-2 text-sm text-gray-600">{modalStory.testimonial}</div>
            <div className="italic text-purple-700 mb-2">“{modalStory.quote}”</div>
            {modalStory.video && <video src={modalStory.video} controls className="w-full rounded mb-2" />}
            <div className="text-xs text-gray-400">Platform: {modalStory.platform} | Industry: {modalStory.industry} | Brand Type: {modalStory.brandType}</div>
          </div>
        </div>
      )}
      {/* CTA Banner */}
      <section className="w-full bg-purple-600 py-10 px-4 flex flex-col items-center text-white">
        <h2 className="text-2xl font-bold mb-2">Start Your Success Story</h2>
        <p className="mb-4">Ready to achieve real results? Join our platform and connect with top creators and brands.</p>
        <button className="px-8 py-3 rounded-lg bg-white text-purple-700 font-semibold hover:bg-purple-100 transition">Sign Up Now</button>
      </section>
