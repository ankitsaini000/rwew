...
interface BrandReviewUpdateModalProps {
  open: boolean;
  onClose: () => void;
  order: Order;
  existingReview: any; // The existing review data
  onReviewUpdated: () => void;
}

const MAX_COMMENT = 1000;

const BrandReviewUpdateModal: React.FC<BrandReviewUpdateModalProps> = ({ 
  open, 
  onClose, 
  order, 
  existingReview, 
  ...
  // Update local state when existingReview changes
  useEffect(() => {
    if (existingReview) {
  ...
  if (!open || !order || !existingReview) return null;
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
      <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full p-0 relative animate-fadeInUp">
        ...
        <div className="px-8 pt-8 pb-6 flex flex-col items-center">
          ...
          />
          <h2 className="text-xl font-bold text-gray-900 mb-1">Update Review</h2>
          ...
          {success && (
            <div className="flex flex-col items-center my-6">
              ...
              <div className="text-green-700 font-semibold mt-2">Review updated!</div>
            </div>
          )}
...
export default BrandReviewUpdateModal; 